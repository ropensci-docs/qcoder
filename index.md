...
can find this with the [`getwd()`](https://rdrr.io/r/base/getwd.html)
command at the console). If you have a specific location where you want
to put the folder change your working directory.
...
- code.description (Longer description of the code, must be enclosed in
  quotation marks.)
...
To import this data into Qcode user the
[`import_project_data()`](https://docs.ropensci.org/qcoder/reference/import_project_data.md)
function.
...
launch the app use the function
[`qcode()`](https://docs.ropensci.org/qcoder/reference/qcode.md).

``` R
qcode()
```

Which will launch this application. If your current working directory is
not the location of your project, use the `use_wd = FALSE` option.
However, on Windows this will not work unless you have set a HOME or
R_USER.
...
backups to save storage space. An important goal is to move to using git
for this purpose.
...
You must go to the first tab of the the qcode application to update
those displays. This is a high priority development item.
...
product. Please see the issue tracker for a list of short-term and
longer-term goals. These goals include interoperability with other QDA
packages.

The most important thing is to have more people try qcoder and give us
feedback! We do not want to release and then discover that our testing
has missed problems that are obvious to our intended user base.
